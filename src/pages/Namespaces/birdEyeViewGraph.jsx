...
const RectangleGraph = ({ data, setXStateCoordiate, setYStateCoordiate }) => {
  ...
  useEffect(() => {
    ...
    if (!xScaleRef.current || !yScaleRef.current) {
      ...
      const padding = 20; // Add padding for spacing
    ...
    const renderRectangles = () => {
      ...
      g.selectAll('rect')
        .data(data)
        .join('rect')
        .attr('x', d => xScale(d.x)) // Scale x position
        .attr('y', d => yScale(d.y)) // Scale y position
        .attr('width', d => xScale(d.x + d.width) - xScale(d.x)) // Scale width
        .attr('height', d => yScale(d.y + d.height) - yScale(d.y)) // Scale height
        .attr('fill', d => d.color)
        .style('stroke', '#fff');
    };
